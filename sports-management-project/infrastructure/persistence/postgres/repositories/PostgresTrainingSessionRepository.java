...
public class PostgresTrainingSessionRepository implements TrainingSessionRepository {
    ...
    @Override
    public List<TrainingSession> findRecentByAthleteId(UUID athleteId, LocalDateTime since) {
        TypedQuery<TrainingSessionEntity> query = entityManager.createQuery(
            "SELECT s FROM TrainingSessionEntity s WHERE s.athleteId = :athleteId AND s.sessionDate >= :since",
    ...
    public TrainingSession save(TrainingSession session) {
        TrainingSessionEntity entity = mapper.toEntity(session);
        if (session.getSessionId().getValue() == null) {
    ...
    @Override
    public List<TrainingSession> findByAthleteId(UUID athleteId) {
        TypedQuery<TrainingSessionEntity> query = entityManager.createQuery(
            "SELECT s FROM TrainingSessionEntity s WHERE s.athleteId = :athleteId ORDER BY s.sessionDate DESC",
            TrainingSessionEntity.class
        );
        query.setParameter("athleteId", athleteId);
        return mapper.toDomainSessionList(query.getResultList());
